...
```{r LoganLocation, fig.cap="The Logan River Watershed.", echo=F, out.width="70%", fig.align='center'}
knitr::include_graphics("./images/Module3/Location Map Logan River.jpg")
```
...
```{r LoganEcoregions, fig.cap="Level IV Ecoregions of the Logan River Watershed. ", echo=F, out.width="70%", fig.align='center'}
knitr::include_graphics("./images/Module3/Ecoregions Logan River.jpg")
```

## Longitudinal Profile

```{r LoganProfile, fig.cap="Profile of the Logan River. The x axis shows distance in meters, while the y axis shows elevation.", echo=F, out.width="70%", fig.align='center'}
knitr::include_graphics("./images/Module3/LoganProfile.png")
```
...
```{r LoganProfileKnickpoints, fig.cap="Profile of the Logan River with knickpoints labelled with a potential explanation.", echo=F, out.width="70%", fig.align='center'}
knitr::include_graphics("./images/Module3/LoganProfile_withknickpoints.png")
```
...
```{r GoldCreekLocation, fig.cap="The Gold Creek-Blackfoot River Watershed. ", echo=F, out.width="70%", fig.align='center'}
knitr::include_graphics("./images/Module3/GoldCreekLocation.jpg")
```
...
```{r DiffGoldCreek, echo=F, message=F, warning=F, fig.caption = "The two subcatchments within the Gold Creek-Blackfoot Catchment have strong differences between their annual precipitation (b) and dominant vegetation (c). Hillshade also shown (a) for reference. "}
library(cowplot)
library(ggplot2)
...
In summary, the watershed has some strong differences between the two subbasins contained within it (Fig. \@ref(fig:DiffGoldCreek)). 
...
```{r GoldCreekEcoregions, fig.cap="Level IV Ecoregions of the Gold Creek-Blackfoot Watershed. ", echo=F, out.width="70%", fig.align='center'}
knitr::include_graphics("./images/Module3/Ecoregions Gold Creek.jpg")
```

## Longitudinal profile

```{r GoldCreekProfile, fig.cap="Profile of the Gold Creek-Blackfoot River Watershed. The x axis shows distance in meters, while the y axis shows elevation.", echo=F, out.width="70%", fig.align='center'}
knitr::include_graphics("./images/Module3/GoldCreekProfile.png")
```
...
There are a series of knickpoints along the profile. In the Blackfoot River section, there used to be a series of dams in this area to support the local mill in Bonner, MT, but those were removed  in 2007. I can't tell where precisely they all used to be, but there are a few knickpoints in this section that are likely due to mills and dams on this stretch. Higher up, there's a knickpoint that coincides with a road crossing. Potentially that road crossing is limiting some flow. Higher still, there are a few more knickpoints, but the origin is less clear. I wonder if the hard rock parent material results in a few knickpoints where the rock is extra hard to erode. Another point that Jordan pointed out is the big change in slope at 8 km: I looked into this an this coincides with a geology change from quartzite dominated (super hard) to metasiltstone and argillite dominated parent material. This likely facilitates the change here from a steep sloped narrow drainage to a more gentle slope with a wider floodplain. 

```{r GoldCreekProfileKnickpoints, fig.cap="Profile of the Gold Creek-Blackfoot River Watershed with knickpoints labelled with a potential explanation.", echo=F, out.width="70%", fig.align='center'}
knitr::include_graphics("./images/Module3/GoldCreekProfile_withknickpoints.png")
```
...
With grading, a couple suggestions came from Jordan: 
- For the form factor, the values from the book (4, 8) were not right, so 0.98 is a significantly higher value for the Gold Creek watershed than for the Logan (0.34). This means Gold Creek is probably more responsive (again, the book is wrong) to runoff events than Logan is. 
- Drainage density showed these two systems to be fairly similar, despite their contrast in geology, but this may be explained by karst occurring in the Logan River watershed, leading to lower amounts of surface runoff overall. 
