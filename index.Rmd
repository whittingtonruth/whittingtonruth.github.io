...
Cover metrics are calculated from LPI pin drops and can either show absolute cover (i.e. the proportion of the plot covered by a given plant or plant group) or relative cover (i.e. the proportion of the vascular species hits made up by a given plant or plant group). Covers calculated as absolute cover are similar to covers calculated from the Terrestrial AIM program. Both Absolute and Relative covers were calculated for the 2020 data, as shown below in two sets of graphs, as we explore what these metrics may indicate. For each type of cover, the data were grouped either by Administrative State, or Wetland Type.

### Absolute Cover Metrics by Administrative State
...
absitems <- list(list(label = "TotalFoliarCover", 
              ...
              list(label = "TotalBasalCover", 
                   args = list(list(visible = c(F, T, F, F, F, F, F, F, F, F, F, F)))),
              list(label = "Absolute Native", 
                   args = list(list(visible = c(F, F, T, F, F, F, F, F, F, F, F, F)))),
              list(label = "Absolute Noxious", 
                   args = list(list(visible = c(F, F, F, T, F, F, F, F, F, F, F, F)))),
              list(label = "Absolute Hydrophyte", 
                   args = list(list(visible = c(F, F, F, F, T, F, F, F, F, F, F, F)))),
              list(label = "Absolute HydroFAC", 
                   args = list(list(visible = c(F, F, F, F, F, T, F, F, F, F, F, F)))),
              list(label = "Absolute Forb", 
                   args = list(list(visible = c(F, F, F, F, F, F, T, F, F, F, F, F)))),
              list(label = "Absolute Graminoid", 
                   args = list(list(visible = c(F, F, F, F, F, F, F, T, F, F, F, F)))),
              list(label = "Absolute Shrub", 
                   args = list(list(visible = c(F, F, F, F, F, F, F, F, T, F, F, F)))),
              list(label = "Absolute Tree", 
                   args = list(list(visible = c(F, F, F, F, F, F, F, F, F, T, F, F)))),
              list(label = "Absolute Annual", 
                   args = list(list(visible = c(F, F, F, F, F, F, F, F, F, F, T, F)))),
              list(label = "Absolute Perennial", 
                   args = list(list(visible = c(F, F, F, F, F, F, F, F, F, F, F, T))))
)
...
stateabscoverplot <- ggplotly(ggplot(alldata) +
    ...
    geom_boxplot(aes(y = TotalBasalCover, x = AdminState))+
      geom_boxplot(aes(y = AbsoluteNativeCover, x = AdminState))+
      geom_boxplot(aes(y = AbsoluteNoxiousCover, x = AdminState))+
      geom_boxplot(aes(y = AbsoluteHydrophyteCover, x = AdminState))+
      geom_boxplot(aes(y = AbsoluteHydroFACCover, x = AdminState))+
      geom_boxplot(aes(y = AbsoluteForbCover, x = AdminState))+
      geom_boxplot(aes(y = AbsoluteGraminoidCover, x = AdminState))+
      geom_boxplot(aes(y = AbsoluteShrubCover, x = AdminState))+
      geom_boxplot(aes(y = AbsoluteTreeCover, x = AdminState))+
      geom_boxplot(aes(y = AbsoluteAnnualCover, x = AdminState))+
      geom_boxplot(aes(y = AbsolutePerennialCover, x = AdminState))+
    xlab("Administrative State") + ylab("Percent Absolute Cover") + 
...
### Absolute Cover Metrics by Wetland Type
...
wetlandtypeabscoverplot <- ggplotly(ggplot(alldata) +
    ...
    geom_boxplot(aes(y = TotalBasalCover, x = WetlandType))+
      geom_boxplot(aes(y = AbsoluteNativeCover, x = WetlandType))+
      geom_boxplot(aes(y = AbsoluteNoxiousCover, x = WetlandType))+
      geom_boxplot(aes(y = AbsoluteHydrophyteCover, x = WetlandType))+
      geom_boxplot(aes(y = AbsoluteHydroFACCover, x = WetlandType))+
      geom_boxplot(aes(y = AbsoluteForbCover, x = WetlandType))+
      geom_boxplot(aes(y = AbsoluteGraminoidCover, x = WetlandType))+
      geom_boxplot(aes(y = AbsoluteShrubCover, x = WetlandType))+
      geom_boxplot(aes(y = AbsoluteTreeCover, x = WetlandType))+
      geom_boxplot(aes(y = AbsoluteAnnualCover, x = WetlandType))+
      geom_boxplot(aes(y = AbsolutePerennialCover, x = WetlandType))+
    xlab("General Wetland Type") + ylab("Percent Absolute Cover") + 
      scale_y_continuous(n.breaks = 20) + 
    theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), axis.text = element_text(size = 12, color = "black"), text = element_text(size = 16), axis.text.x = element_text(angle = 25)))%>%
  layout(
...
### Relative Cover Metrics by Administrative State

Use the dropdown box to select the cover metric you wish to explore and click on the graph as needed to zoom in.
```{r adminstate x relative cover, echo = F, warning=F, out.width="100%"}
items <- list(list(label = "TotalAbsoluteCover", 
                   args = list(list(visible = c(T, F, F, F, F, F, F, F, F, F, F)))),
              list(label = "Relative Native", 
                   args = list(list(visible = c(F, T, F, F, F, F, F, F, F, F, F)))),
              list(label = "Relative Noxious", 
...
statecoverplot <- ggplotly(ggplot(alldata) +
      ...
      geom_boxplot(aes(y = RelativeNoxiousCover, x = AdminState))+
    ...
    xlab("Administrative State") + ylab("Percent Relative Cover") + 
      scale_y_continuous(n.breaks = 20) + 
    theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), axis.text = element_text(size = 12, color = "black"), text = element_text(size = 16)))%>%
  layout(
...
### Relative Cover Metrics by Wetland Type

Use the dropdown box to select the cover metric you wish to explore and click on the graph as needed to zoom in.
```{r wetland type x relative cover, echo = F, warning=F, out.width="100%"}
wetlandtypecoverplot <- ggplotly(ggplot(alldata) +
      ...
      geom_boxplot(aes(y = RelativeNoxiousCover, x = WetlandType))+
    ...
    xlab("General Wetland Type") + ylab("Percent Relative Cover") + 
      scale_y_continuous(n.breaks = 20) + 
    theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), axis.text = element_text(size = 12, color = "black"), text = element_text(size = 16), axis.text.x = element_text(angle = 25)))%>%
  layout(
...
### Non-Plant Cover Metrics by Administrative State
...
### Non-Plant Cover Metrics by Wetland Type
...
Data collection on hummocks is intended to characterize the physical structure of hummocks within the plot and to help detect changes due to livestock use, erosion, hydrologic modification, and/or changes in the biotic community over time. Hummock methods are a contingent method, and only occur in a small portion of Riparian and Wetland AIM plots. In 2020, 19 sites had one or more hummocks measured along transects. Boxplots shown demonstrate the distribution of each metric within the sites where at least one hummock was measured. 
